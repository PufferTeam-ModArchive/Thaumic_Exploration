...
public class TileEntityFloatyCandle extends TileEntity {
    ...
    // TileInfusionMatrix tileMatrix = (TileInfusionMatrix) this.worldObj.getBlockTileEntity(matrix.posX,
    // matrix.posY, matrix.posZ);
    // if (tileMatrix != null) {
    // if (!((ArrayList<ChunkCoordinates>)(ReflectionHelper.getPrivateValue(TileInfusionMatrix.class, tileMatrix,
    // "pedestals"))).contains(new ChunkCoordinates(this.xCoord,this.yCoord,this.zCoord))) {
    // ArrayList<ChunkCoordinates> pedestals = ReflectionHelper.getPrivateValue(TileInfusionMatrix.class,
    // tileMatrix, "pedestals");
