...
// {
// if ((event.source == DamageSource.drown) || (event.source == DamageSource.wither) || (event.source ==
// DamageSource.outOfWorld) || (event.source == DamageSource.starve)) {
// return;
// }
...
// int feedEmerg = 0;
// for (int a = 0; a < 4; a++) {
// if ((player.inventory.armorItemInSlot(a) != null) && ((player.inventory.armorItemInSlot(a).getItem()
// instanceof ItemEnhancedRunicArmor)))
// {
//
...
//
// player.inventory.armorItemInSlot(((Integer)runic.get(index)).intValue()).setItemDamage(player.inventory.armorItemInSlot(((Integer)runic.get(index)).intValue()).getItemDamage()
// + 1);
//
// event.ammount -= 1.0F;
// if (player.inventory.armorItemInSlot(((Integer)runic.get(index)).intValue()).getItemDamage() >=
// player.inventory.armorItemInSlot(((Integer)runic.get(index)).intValue()).getMaxDamage()) {
// runic.remove(index);
//
// System.out.println(runic.size());
// }
...
// player.worldObj.newExplosion(player, player.posX, player.posY + player.height / 2.0F, player.posZ, 1.0F +
// feedDes * 0.5F, false, false);
// }
// else
// {
// System.out.println((runic.size() == 0) + " " + (feedDes > 0) + " " +
// (!ItemRunicArmor.upgradeCooldown.containsKey(key)));
// }
...
//
// player.inventory.armorItemInSlot(((Integer)runicTemp.get(index)).intValue()).setItemDamage(player.inventory.armorItemInSlot(((Integer)runicTemp.get(index)).intValue()).getItemDamage()
// - 1);
//
// count--;
